...
class TicketSerializer(serializers.ModelSerializer):
    ...
    assigned_agent_id = serializers.PrimaryKeyRelatedField(queryset=Agent.objects.all(), source="assigned_agent", write_only=True)
    severity_display = serializers.CharField(source='get_severity_display', read_only=True)
    status_display = serializers.CharField(source='get_status_display', read_only=True)
    
    class Meta:
        model = Ticket
        fields = ['id', 'title', 'description', 'requester', 'created_at', 'severity', 'severity_display', 'assigned_agent', 'assigned_agent_id', 'status_display', 'status', 'resolved_at']
        read_only_fields = ['id', 'created_at', 'resolved_at', 'severity_display', 'status_display', 'assigned_agent']
