...
const Sidebar = () => {
    ...
    <div key={section} className='flex flex-col font-medium py-2'>
        ...
        <div className='flex flex-col'>
            {items.map(({label, link, icon: Icon}) => <Link key={label} to={link} className={`flex gap-4 px-4 py-2 rounded-sm hover:bg-main-dark text-text/75`}>{Icon && <Icon width={16} />} {label}</Link>)}
        </div>
    </div>)
